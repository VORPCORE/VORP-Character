...
namespace VorpCharacter.Menus
{
    class ClothesMenu
    {
        ...
        private static void SetupMenu()
        {
            ...
            List<string> bowType = new List<string>();
            bowType.Add(Common.GetTranslation("NobowValue"));
            ...
            else
            {
                ...
                for (float i = 1; i < SkinsUtils.BOW_FEMALE.Count + 1; i++)
                {
                    bowType.Add(Common.GetTranslation("bowValue") + i);
                }
            }
            MenuListItem mListbow = new MenuListItem(Common.GetTranslation("bow"), bowType, 0, Common.GetTranslation("bowDesc"));
            clothesMenu.AddMenuItem(mListbow);


            List<string> teethType = new List<string>();
            teethType.Add(Common.GetTranslation("NoteethValue"));
            if (CreateCharacter.model_selected == "mp_male")
            {

                for (float i = 1; i < SkinsUtils.TEETH_MALE.Count + 1; i++)
                {
                    teethType.Add(Common.GetTranslation("teethValue") + i);
                }
            }
            else
            {

                for (float i = 1; i < SkinsUtils.TEETH_FEMALE.Count + 1; i++)
                {
                    teethType.Add(Common.GetTranslation("teethValue") + i);
                }
            }
            MenuListItem mListteeth = new MenuListItem(Common.GetTranslation("teeth"), teethType, 0, Common.GetTranslation("teethDesc"));
            clothesMenu.AddMenuItem(mListteeth);

            List<string> armorType = new List<string>();
            armorType.Add(Common.GetTranslation("NoarmorValue"));
            if (CreateCharacter.model_selected == "mp_male")
            {

                for (float i = 1; i < SkinsUtils.ARMOR_MALE.Count + 1; i++)
                {
                    armorType.Add(Common.GetTranslation("armorValue") + i);
                }
            }
            else
            {

                for (float i = 1; i < SkinsUtils.ARMOR_FEMALE.Count + 1; i++)
                {
                    armorType.Add(Common.GetTranslation("armorValue") + i);
                }
            }
            MenuListItem mListarmor = new MenuListItem(Common.GetTranslation("armor"), armorType, 0, Common.GetTranslation("armorDesc"));
            clothesMenu.AddMenuItem(mListarmor);
            ...
            clothesMenu.OnListIndexChange += (_menu, _listItem, _oldIndex, _newIndex, _itemIndex) =>
            {
                switch (_itemIndex)
                {
                    ...
                    case 30:
                        ...
                        break;
                    case 31:
                        CreateCharacter.SetPlayerComponent(_newIndex, 0x8E84A2AA, "bow", SkinsUtils.BOW_FEMALE, SkinsUtils.BOW_FEMALE);
                        break;
                    case 32:
                        CreateCharacter.SetPlayerComponent(_newIndex, 0x96EDAE5C, "teeth", SkinsUtils.TEETH_MALE, SkinsUtils.TEETH_FEMALE);
                        break;
                    case 33:
                        CreateCharacter.SetPlayerComponent(_newIndex, 0x72E6EF74, "armor", SkinsUtils.ARMOR_MALE, SkinsUtils.ARMOR_FEMALE);
                        break;
                }
            };
